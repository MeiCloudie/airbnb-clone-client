...
interface UploadImageRoomDialogProps {
  isOpen: boolean
  onClose: () => void
  roomId: number // Sử dụng roomId để upload ảnh theo phòng thuê
}

export default function UploadImageRoomDialog({ isOpen, onClose, roomId }: UploadImageRoomDialogProps) {
  const [preview, setPreview] = React.useState<string | ArrayBuffer | null>('')

  const { showNotification } = useToastifyNotification()
  const { postImageRoom, isLoading, getAllRooms } = useRoom() // Sử dụng store
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    const error = await postImageRoom({ maPhong: roomId.toString() }, values.image)

    if (!error) {
      showNotification(`Cập nhật ảnh phòng thuê thành công!`, 'success')
      onClose()
      await getAllRooms()
    } else {
      showNotification(`Cập nhật ảnh phòng thuê thất bại: ${error.message}`, 'error')
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className='uppercase'>Cập Nhật Ảnh Phòng Thuê</DialogTitle>
