...
import { PutRoomPayload, Room } from '@/types/room.type'
...
export default function UpdateRoomDialog({ isOpen, onClose, roomData }: UpdateRoomDialogProps) {
  ...
  const form = useZodForm(roomUpdateSchema, {
    defaultValues: {
  ...
  const onSubmit = async (data: PutRoomPayload) => {
    const error = await putRoom(data)

    if (error) {
      showNotification(`Cập nhật phòng thất bại: ${error.content}`, 'error')
    } else {
      showNotification('Phòng đã được cập nhật thành công!', 'success')
      await getAllRooms()
      form.reset()
      onClose()
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='h-full md:max-h-[80vh]'>
        <DialogHeader>
          <DialogTitle className='uppercase'>Cập Nhật Phòng</DialogTitle>
